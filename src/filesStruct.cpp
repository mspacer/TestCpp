...
void read_from_file() {
  ...
  if (!(fp = fopen("fp.dat", "rb"))) {
    ...
    while (!feof(fp)) {  //виведення даних з файлу
...
void list_student() {
  ...
  if (!(fp = fopen("fp.dat","rb"))) {
  //перевірка існування файлу при відкритті
    cerr<<"File do not exist!";
  } else {
  ...
  if (!(fp = fopen("fp.dat","rb"))) {
    ...
    while(!feof(fp)) {
      ...
      //перегляд студентів
        ...
        //сума балів студентів по дисципліні
      sr[i]=float(sr[i])/predmet[i].stnum;
      //середній бал по даному предмету
    ...
    for(i=1;i<kol_prep;i++) {  //перегляд дисциплін
        if (max<sr[i]) {
          max=sr[i];
          //визначити максимальний бал й номер дисципліни
          nomer_predmet=i;
        }
    }
...
void stipendia() {
  float sum=0;
  //сума стипендій студентів с середнім балом менше 3
  int k=0;
  //кількість записів про студентів у вхідному масиві
  int l=0;       //лічильник всіх студентів
  subject predmet;        //предмет
  stud s,tmp[10],stud_tmp[10];

  //проміжні масиви студентів
  ...
  if (!(fp=fopen("fp.dat","rb"))) {
    ...
    //лічильник предметів, що вивчає один студент
    for (i=0;i<k;i++) { //перегляд всього масиву студентів
      stud_tmp[l].bal=0;
      //сума балів одного студента по різним предметам
      kol[l]=0;
      //кількість предметів для одного студента
      int f=0;     //прапорець обробки даних по студенту
      for (j=0;j<i;j++) {
        //перегляд студентів спочатку масиву до даного запису
        if ((strcmp(tmp[i].fio,tmp[j].fio)==0))
            f=1;        //студент вже зустрічався в масиві
      }

      if (f==0) { //студент ще не зустрічався в масиві
        for (j=i;j<k;j++) { // перегляд масиву від даного запису до кінця
          if (strcmp(tmp[i].fio,tmp[j].fio) == 0) {
          //чи є той самий студент по іншому предмету
            stud_tmp[l].bal+=tmp[j].bal;
            //накопичення балів по одному студенту
            strcpy(stud_tmp[l].fio, tmp[j].fio);
            //запам'ятати студента
            stud_tmp[l].stip=tmp[j].stip;
            //запам'ятати стипендію
    ...
    //сортування студентів за абеткою
    ...
    //виведення даних про загальний середній бал
    //та початкову стипендію
...
void fond() {
  ...
  if (!(fp=fopen("fp.dat","rb"))) {
    ...
    while(!feof(fp)) {
      ...
      //перегляд студентів по даній дисципліні
      {                  //копіювання масиву студентів
        tmp[k]=predmet.student[j];
        k++;
      }
    ...
    for (i=0;i<k;i++) { //перегляд всього масиву студентів
      int f=0;      //прапорець обробки даних по студенту
      for (j=0;j<i;j++) {//перегляд студентів спочатку масивудо даного запису
        if ((strcmp(tmp[i].fio,tmp[j].fio)==0))
            f=1;        //студент вже зустрічався в масиві
      }
      if (f==0)  //студент ще не зустрічався в масиві
